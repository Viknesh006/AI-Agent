...
import { ArrowRight, Zap, ChevronRight, Check, Loader2 } from "lucide-react";
...
// ── Razorpay checkout button used in the landing page pricing section ──────────
declare global {
  interface Window {
...
function LandingRazorpayButton({
  planKey,
  planLabel,
  razorpayPrice,
  isDowngrade,
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: {
          ...
          toast.success(`🎉 Upgraded to ${planLabel}! Credits topped up.`);
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>{isDowngrade ? "Downgrade" : "Get started"} — ₹{razorpayPrice}/mo<ArrowRight className="h-3.5 w-3.5" /></>
      )}
    </Button>
  );
}
