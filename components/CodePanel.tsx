...
// ─── File Normalizer & Code Sanitizer ─────────────────────────────────────────

function sanitizeCode(code: string, filepath: string): string {
  if (!code || typeof code !== "string") return code || "";
  let clean = code;

  // 1. Transform lucide-react imports into safe icon getters (prevents undefined icon crashes inside Hero/App)
  clean = clean.replace(
    ...
    (match, p1) => {
      const items = p1
        ...
        .map((item: string) => {
          ...
          const baseName =
            name.endsWith("Icon") && name.length > 4 ? name.slice(0, -4) : name;
          return { name, alias, baseName };
        })
        .filter(Boolean);

      const declarations = (
        items as { name: string; alias: string; baseName: string }[]
      )
        .map(
          ({ name, alias, baseName }) =>
            `const ${alias} = _getLucideIcon("${name}", "${baseName}");`
        )
        .join("\n");

      return `import * as _LucideAll from "lucide-react";\nconst _getLucideIcon = (n, b) => _LucideAll[n] || _LucideAll[b] || _LucideAll.Sparkles || _LucideAll.HelpCircle || (() => null);\n${declarations}`;
    }
  );

  // 2. Ensure component files export both default and named exports
  if (
    filepath !== "/index.js" &&
    filepath !== "/styles.css" &&
    ...
    const fnMatches = Array.from(
      clean.matchAll(/(?:export\s+)?(?:default\s+)?(?:function|class)\s+([A-Z][A-Za-z0-9_$]*)/g)
    ).map((m) => m[1]);

    const constMatches = Array.from(
      clean.matchAll(/^export\s+(?:const|let|var)\s+([A-Z][A-Za-z0-9_$]*)\s*=/gm)
    ).map((m) => m[1]);

    const allCompNames = Array.from(new Set([...fnMatches, ...constMatches]));

    if (allCompNames.length > 0) {
      const primary = allCompNames[0];
      ...
      for (const name of allCompNames) {
        const hasNamed =
          ...
          new RegExp(`export\\s+(?:const|function|class|let|var)\\s+${name}\\b`).test(clean);
        if (!hasNamed) {
          clean += `\nexport { ${name} };\n`;
        }
      }
...
function normalizeFiles(
  rawFiles: Record<string, { code: string } | string> | null | undefined
  ...
  for (const [rawPath, content] of Object.entries(rawFiles)) {
    ...
    const code = sanitizeCode(rawCode, cleanPath);
    normalized[cleanPath] = { code };
  }

  if (!normalized["/App.js"]) {
    const altAppKey = Object.keys(normalized).find((k) => {
    ...
    if (altAppKey) {
      normalized["/App.js"] = normalized[altAppKey];
    }
  }

  // Auto-generate safe fallback components for any imported local files that were missing
  const existingPaths = new Set(Object.keys(normalized));
  for (const { code } of Object.values(normalized)) {
    ...
    for (const match of matches) {
      ...
      const fullPath = `/${cleanRel}`;
      if (!existingPaths.has(fullPath)) {
        const compName =
          fullPath.split("/").pop()?.replace(/\.[a-z0-9]+$/i, "") || "Component";
        const pascalName = compName.charAt(0).toUpperCase() + compName.slice(1);
        normalized[fullPath] = {
          code: `import React from 'react';\nexport default function ${pascalName}() {\n  return <div className="p-4 border border-dashed border-slate-700 rounded text-slate-400 text-sm">${pascalName} Component</div>;\n}\nexport { ${pascalName} };\n`,
        };
  ...
  // Inject custom /index.js with React.createElement safeguard
  normalized["/index.js"] = {
    code: `import React from "react";
...
if (React && typeof React.createElement === "function") {
  ...
  React.createElement = function (type, props, ...children) {
    if (type === undefined || type === null) {
      const fallbackName = props?.name || "Component";
...
function SandpackInner({
  ...
  useEffect(() => {
    ...
    const normalized = normalizeFiles(fileData.files);
    ...
    prevFilesRef.current = normalized;
...
export function CodePanel({
  ...
  const files = normalizeFiles(fileData?.files);
  const dependencies = {
    ...BASE_DEPENDENCIES,
    ...(fileData?.dependencies ?? {}),
  };

  // Key on fileData presence + sorted file paths + content lengths + sorted dependencies.
  // This ensures Sandpack remounts cleanly with fresh code when generation finishes.
  const filePathKey = fileData
    ? "data|" +
      Object.keys(files).sort().join("|") +
      "|" +
      Object.entries(files)
        .map(([k, v]) => `${k}:${v.code.length}`)
        .join("|") +
      "|" +
